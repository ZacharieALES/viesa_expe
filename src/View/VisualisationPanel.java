...
import experesults.ChangeAlignmentStatus;
import experesults.DisplayAlignmentAction;
...
public class VisualisationPanel extends JPanel implements ActionListener{
	...
	public XRadioButton[] rButtons = new XRadioButton[4];
	...
	private VisualisationPanel(){
		...
		this.setLayout(new MigLayout("fill", "[center][][]", "0[grow][]"));
		...
		TitledBorder tb_change_status = BorderFactory.createTitledBorder("Alignment status");

		aaTablePanelAB.setBorder(tb_change_status);

		rButtons[0] = new XRadioButton(new ImageIcon(ClassLoader.getSystemResource("img/unknown.png")), "(A)");
		rButtons[1] = new XRadioButton(new ImageIcon(ClassLoader.getSystemResource("img/star2.png")), "(Z)");
		rButtons[2] = new XRadioButton(new ImageIcon(ClassLoader.getSystemResource("img/star.png")), "(E)");
		rButtons[3] = new XRadioButton(new ImageIcon(ClassLoader.getSystemResource("img/trash.png")), "(R)");

		rButtons[0].getRadioButton().setActionCommand("0");
		rButtons[1].getRadioButton().setActionCommand("1");
		rButtons[2].getRadioButton().setActionCommand("2");
		rButtons[3].getRadioButton().setActionCommand("3");

		rButtons[0].getRadioButton().addActionListener(this);
		rButtons[1].getRadioButton().addActionListener(this);
		rButtons[2].getRadioButton().addActionListener(this);
		rButtons[3].getRadioButton().addActionListener(this);

		ButtonGroup group1 = new ButtonGroup();
		group1.add(rButtons[0].getRadioButton());
		group1.add(rButtons[1].getRadioButton());
		group1.add(rButtons[2].getRadioButton());
		group1.add(rButtons[3].getRadioButton());

		aaTablePanelAB.add(rButtons[0]);
		aaTablePanelAB.add(rButtons[1]);
		aaTablePanelAB.add(rButtons[2]);
		aaTablePanelAB.add(rButtons[3], "wrap");
		...
		trash.setText("Irrelevant alignment");
		star.setText("Relevant and optimal alignment");
		star2.setText("Relevant and non optimal alignment");
		unknown.setText("Unevaluated alignment");
	...
	public void displayAlignment(Alignment a){
		...
		if(Corpus.getCorpus().results != null)
			Corpus.getCorpus().results.addAction(new DisplayAlignmentAction(a));
		...
		if(id1 >= 0){			
			...
			switch(a.status){
			case UNKNOWN: rButtons[0].getRadioButton().setSelected(true); jib_left12.setImage("img/unknown.png"); jib_right12.setImage("img/unknown.png"); break;
			case RELEVANT_AND_NOT_OPTIMAL: rButtons[1].getRadioButton().setSelected(true); jib_left12.setImage("img/star2.png");  jib_right12.setImage("img/star2.png");break;
			case RELEVANT_AND_OPTIMAL: rButtons[2].getRadioButton().setSelected(true); jib_left12.setImage("img/star.png"); jib_right12.setImage("img/star.png"); break;
			case IRRELEVANT: rButtons[3].getRadioButton().setSelected(true); jib_left12.setImage("img/trash.png");  jib_right12.setImage("img/trash.png");break;
			}
		}
		...
		if(tree.getLastSelectedPathComponent() instanceof DefaultMutableTreeNode){
			...
			if(cNode.getUserObject() instanceof Cluster){
				...
				//			   else if(arg0.getClickCount() == 1)
				{
					...
					//					if(isLeftClick)
					{			   
						Alignment a = ((Cluster)cNode.getUserObject()).getAlignment();
						displayAlignment(a);

						//						System.out.println("--------\nAlignment: ");						
						//						System.out.println("id: " + a.getP1().getIndex() + " " + a.getP1().getBounds()[0] + "/" + a.getP1().getBounds()[1]);
						//						System.out.println("id: " + a.getP2().getIndex() + " " + a.getP2().getBounds()[0] + "/" + a.getP2().getBounds()[1]);
					}
					//					else
					{
		...
		if(csAlignments != null){			
			...
			if(csAlignments.size() > 0){
				while(a == null && id < csAlignments.get(0).getClusterSet().size()){
					...
					if(csAlignments.get(0).getCluster(id).getAlignment().status == PatternStatus.UNKNOWN)
						a = csAlignments.get(0).getCluster(id).getAlignment();
			...
			if(csAlignments.size() > 1 && a == null){
				...
				while(a == null && id < csAlignments.get(1).getClusterSet().size()){
					...
					if(csAlignments.get(1).getCluster(id).getAlignment().status == PatternStatus.UNKNOWN)
						a = csAlignments.get(1).getCluster(id).getAlignment();
			...
			if(a != null){
				...
				if(csAlignments.get(0).getClusterSet().size() > 0){
					a = csAlignments.get(0).getCluster(0).getAlignment();
					if(a.status != PatternStatus.IRRELEVANT)
						displayClusterSelection(expeTreeRelevant, 1);
					else
						displayClusterSelection(expeTreeIrrelevant, 1);
				}
				else{
					a = csAlignments.get(1).getCluster(0).getAlignment();
					if(a.status != PatternStatus.IRRELEVANT)
						displayClusterSelection(expeTreeRelevant, 1);
					else
						displayClusterSelection(expeTreeIrrelevant, 1);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getActionCommand().length() == 1){
			...
			if(table1.getPattern() != null){
				...
				PatternStatus previousStatus = p.alignment.status;

				switch(i1){
				case 0: p.alignment.status = PatternStatus.UNKNOWN; tree = expeTreeUnknown; jib_left12.setImage("img/unknown.png");jib_right12.setImage("img/unknown.png");break;
				case 1: p.alignment.status = PatternStatus.RELEVANT_AND_NOT_OPTIMAL; tree = expeTreeRelevant; jib_left12.setImage("img/star2.png"); jib_right12.setImage("img/star2.png");break;
				case 2: p.alignment.status = PatternStatus.RELEVANT_AND_OPTIMAL; tree = expeTreeRelevant; jib_left12.setImage("img/star.png"); jib_right12.setImage("img/star.png");break;
				case 3: p.alignment.status = PatternStatus.IRRELEVANT; tree = expeTreeIrrelevant; jib_left12.setImage("img/trash.png"); jib_right12.setImage("img/trash.png");break;
				}
				
				ExperienceResults results = null;
				
				if(!MainTutorial.IS_TUTO && Corpus.getCorpus().results != null)
					results = Corpus.getCorpus().results;
				else if(MainTutorial.IS_TUTO && MainTutorial.results != null)
					results = MainTutorial.results;

				if(results != null){
					/* If the status has changed */
					if(previousStatus != p.alignment.status){
						switch(previousStatus){
						case IRRELEVANT:
							results.getIrrelevantAlignments().remove(p.alignment);
							break;
						case RELEVANT_AND_NOT_OPTIMAL:
							results.getRelevantNonOptimalAlignments().remove(p.alignment);
							break;
						case RELEVANT_AND_OPTIMAL:
							results.getOptimalAlignments().remove(p.alignment);
							break;
						}

						switch(p.alignment.status){
						case IRRELEVANT:
							results.getIrrelevantAlignments().add(p.alignment);
							break;
						case RELEVANT_AND_NOT_OPTIMAL:
							results.getRelevantNonOptimalAlignments().add(p.alignment);
							break;
						case RELEVANT_AND_OPTIMAL:
							results.getOptimalAlignments().add(p.alignment);
							break;
						}
					}

					results.addAction(new ChangeAlignmentStatus(p.alignment, p.alignment.status));
				}
			...
			if(csAlignments.size() > 1){
				...
				/* If an alignment has been moved into a tree */
				if(tree != null && a != null){

					expeTreeIrrelevant.clearSelection();
					expeTreeRelevant.clearSelection();
					expeTreeUnknown.clearSelection();
					
					/* Select this alignment in this tree */
					tree.selectNode(a);
				}
			}
		}
	...
	public void keyPressed(java.awt.event.KeyEvent keyEvent) {
		...
		if(StandardView.getInstance().isCtrlPressed)
			if(keyEvent.getKeyCode() == KeyEvent.VK_LEFT){
				...
				if(MainTutorial.IS_TUTO){
					if(MainTutorial.getCurrentStep() != null && MainTutorial.getCurrentStep() instanceof ChangeDisplayedAlignment){
						ChangeDisplayedAlignment cda = (ChangeDisplayedAlignment)MainTutorial.getCurrentStep();
						cda.counter++;
						
						if(cda.counter >= cda.numberOfAlignmentsDisplayedToFinish)
		...
		case KeyEvent.VK_A: 
			rButtons[0].getRadioButton().doClick();
			if(MainTutorial.IS_TUTO && MainTutorial.getCurrentStep() != null && MainTutorial.getCurrentStep() instanceof EvaluateAlignment){
				EvaluateAlignment ea = (EvaluateAlignment)MainTutorial.getCurrentStep();
				ea.usedShortcutA = true;
			...
			break;
		case KeyEvent.VK_Z: 
			rButtons[1].getRadioButton().doClick(); 
			if(MainTutorial.IS_TUTO && MainTutorial.getCurrentStep() != null && MainTutorial.getCurrentStep() instanceof EvaluateAlignment){
				EvaluateAlignment ea = (EvaluateAlignment)MainTutorial.getCurrentStep();
				ea.usedShortcutZ = true;
			...
			break;
		case KeyEvent.VK_E: 
			rButtons[2].getRadioButton().doClick(); 
			if(MainTutorial.IS_TUTO && MainTutorial.getCurrentStep() != null && MainTutorial.getCurrentStep() instanceof EvaluateAlignment){
				EvaluateAlignment ea = (EvaluateAlignment)MainTutorial.getCurrentStep();
				ea.usedShortcutE = true;
			...
			break;
		case KeyEvent.VK_R: 
			rButtons[3].getRadioButton().doClick(); 
			if(MainTutorial.IS_TUTO && MainTutorial.getCurrentStep() != null && MainTutorial.getCurrentStep() instanceof EvaluateAlignment){
				EvaluateAlignment ea = (EvaluateAlignment)MainTutorial.getCurrentStep();
				ea.usedShortcutR = true;
