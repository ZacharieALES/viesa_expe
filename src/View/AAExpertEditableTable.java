...
public class AAExpertEditableTable extends JTable implements ComponentListener{

	private int jspWidth;
	private JScrollPane jsp_parent;

	private AAEditableTableModel aaTableModel = null;
	...
	private Color selectedColor = new Color(160, 160, 236);
	private Color selectedPatternColor = new Color(255, 255, 0);
	private Color tableColor;

	public AAExpertEditableTable(AnnotatedArray aa) throws InvalidNumberOfColumnsInInputFilesException{
	...
	private MouseAdapter getMouseAdapter(){
		return new java.awt.event.MouseAdapter() {
			...
			public void mouseClicked(java.awt.event.MouseEvent evt) {
				...
				int row = AAExpertEditableTable.this.rowAtPoint(evt.getPoint());
				int col = AAExpertEditableTable.this.columnAtPoint(evt.getPoint());

				if (row >= 0 && col >= 0) {

					TableModel model = AAExpertEditableTable.this.getModel();

					if(model instanceof AAEditableTableModel){

						View.AAExpertEditableTable.AAEditableTableModel.AAEditableCellModel cell = ((AAEditableTableModel)model).cellData[row][col];
						Pattern p = ((AAEditableTableModel)model).pattern;
						cell.hasBeenEdited = !cell.hasBeenEdited;

						if(cell.hasBeenEdited){

							if(cell.isPatternCell){

								if(p.expertAddedCoordinates == null)
									p.expertAddedCoordinates = new ArrayList<>();

								p.expertAddedCoordinates.add(new PointXMLSerializable(col, row));
							}
							else{

								if(p.expertRemovedCoordinates == null)
									p.expertRemovedCoordinates = new ArrayList<>();

								p.expertRemovedCoordinates.add(new PointXMLSerializable(col, row));
							...
							if(cell.isPatternCell){
								...
								int index = p.expertAddedCoordinates.indexOf(new PointXMLSerializable(col, row));

								if(index == -1)
									System.err.println("Error: unable to remove coordinate from the expert added coordinates");
								else
									p.expertAddedCoordinates.remove(index);
							}
							else{
								...
								int index = p.expertRemovedCoordinates.indexOf(new PointXMLSerializable(col, row));

								if(index == -1)
									System.err.println("Error: unable to remove coordinate from the expert removed coordinates");
								else
									p.expertRemovedCoordinates.remove(index);
		...
		if(aaTableModel != null){
			...
			for(int i = 0 ; i < aaTableModel.getColumnCount() ; i++){
				...
				for(int j = 0 ; j < Math.min(aaTableModel.getRowCount(), 100) ; j++){
					...
					AAEditableTableModel.AAEditableCellModel cell = (AAEditableTableModel.AAEditableCellModel)(aaTableModel.getValueAt(j, i));
...
//				columnSize[columnSize.length-1] += width - currentSize;
	...
	public void setPattern(Pattern p){
		...
		if(aaTableModel == null){
			aaTableModel = new AAEditableTableModel(p);
	...
	public void setAA(AnnotatedArray aa){
		...
		if(aaTableModel == null){
			aaTableModel = new AAEditableTableModel(aa);
			this.setModel(aaTableModel);
			aaTableModel.fireTableDataChanged();
			this.reinitialise();
		}
	...
	private class AAEditableTableModel extends AbstractTableModel {
		...
		private AAEditableCellModel[][] cellData;
		private Pattern pattern = null;
		private String[] columnNames;
		private int nbOfAnnotationColumns;
		private int nbOfCommentsColumns;

		public AAEditableTableModel(AnnotatedArray aa){
			setAA(aa);
		}

		public AAEditableTableModel(Pattern p){
			setPattern(p);
		}
		...
		public void setAA(AnnotatedArray aa){
			...
			if(Corpus.getCorpus().isColumnHeaderDefined())
				headerHeight = 20;
			AAExpertEditableTable.this.getTableHeader().setPreferredSize(new Dimension(jsp_parent.getWidth(),headerHeight));
			...
			AAEditableCellModel[][] newCellData = new AAEditableCellModel[aa.getNumberOfLines()][newNbOfAnnotationColumns + newNbOfCommentsColumns];
			...
			for(int i = 0 ; i < aa.getNumberOfLines() ; i++){
				...
				for(Integer j : commentColumnsIndex){
					...
					newCellData[i][j] = new AAEditableCellModel(aa.getAnnotation(i, j), false);
					if(i%2 == 0)
				...
				for(Integer j : annotationColumnsIndex){
					...
					newCellData[i][j] = new AAEditableCellModel(aa.getAnnotation(i, j), true);
		...
		public void setPattern(Pattern p){
			...
			if(pattern.expertAddedCoordinates != null){
				for(PointXMLSerializable point: pattern.expertAddedCoordinates){

					TableModel model = AAExpertEditableTable.this.getModel();

					if(model instanceof AAEditableTableModel)
						((AAEditableTableModel)model).cellData[point.getY()][point.getX()].hasBeenEdited = true;
				}
			}
			
			if(pattern.expertRemovedCoordinates != null){
				for(PointXMLSerializable point: pattern.expertRemovedCoordinates){
		...
		public void computeDisplayedValue(){
			...
			for(int i = 0 ; i < cellData.length ; i++){
				...
				for(int j = 0 ; j < cellData[0].length ; j++){
					...
					AAEditableCellModel cell = cellData[i][j];
		...
		public class AAEditableCellModel{
			...
			public AAEditableCellModel(String data, boolean isAnnotation){
				this.data = data;
	...
	private class AATableRenderer extends DefaultTableCellRenderer { 
		public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column){
			...
			if(value instanceof AAEditableTableModel.AAEditableCellModel){

				AAEditableTableModel.AAEditableCellModel cell = (AAEditableTableModel.AAEditableCellModel)value;
				...
				if(AAExpertEditableTable.this.getRowHeight(row) != cell.getRowHeight())
					AAExpertEditableTable.this.setRowHeight(row, cell.getRowHeight());
